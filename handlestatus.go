...
func init() {
	...
	h.CommandParser = func(cmd *Command) (string, bool) {
		pattFn := map[*regexp.Regexp]func([]string) (string, bool){
			// Get status
			regexp.MustCompile(`^status ([a-zA-Z0-9\.\-]+)$`): func(s []string) (string, bool) {
				d := Resource{
					Scheme:   "http",
					Host:     server.host,
					Port:     server.port,
			...
			// Status help
			regexp.MustCompile(`^status /\?$`): func(s []string) (string, bool) {
				return "", false
			},
		}
...
func (c *StatusCmd) Get(d Resource) (string, bool) {
	u := new(url.URL)
	u.Scheme = d.Scheme
